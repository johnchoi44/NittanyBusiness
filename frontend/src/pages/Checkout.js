...
const CheckOut = () => {
    ...
    useEffect(() => {
        ...
        fetchCreditCardInfo();
    }, [navigate, state, userEmail, useNew]);

    if (!state?.order) return null;

    const { date, listing_id, seller_email, quantity, total_price } = state.order;
            ...
            <div className="checkout-container">
                <div className="order-preview-container">
                    ...
                    <div className="payment-list-container">
                        {useNew ? (
                            <button id="close-btn" onClick={handleClick}>&#10005;</button>
                        ) : (
                            <button id="new-btn" onClick={handleClick}>
                                <p>&#43;</p>
                                <p>Use New Credit Card</p>
                            </button>
                        )}
                                ...
                                <form onSubmit={handleConfirm}>
                                    <fieldset>
                                        <div className="form-select">
                                            <p className="card-form-label">Select Card Type</p>
                                            <select
                                                value={cardType}
                                        ...
                                        <p className="card-form-label">Credit Card Number</p>
                                        <input
                                            type="text"
                                            value={creditNum}
                                        ...
                                        <p className="card-form-label">Expiration Date</p>
                                        <input
                                            type="month"
                                            value={expDate}
                                        ...
                                        <p className="card-form-label">CVV</p>
                                        <input
                                            type="number"
                                            value={cvv}
                                            placeholder="CVV"
                                        ...
                                        <button type="submit" id="confirm-btn" style={{ marginTop: "20px" }}>Save New Card and Pay</button>
                                    </fieldset>
                                </form>
                            </div>
                        )}
                    ...
                    <div className="item-container">
                        ...
                        <p />
                        <p style={{ justifySelf: "flex-end" }}>${total_price}</p>
                    </div>

                    {!useNew && cards.length > 0 && (
                        <button id="confirm-btn" onClick={handleConfirm}>Confirm and Pay</button>
                    )}
                </div>
            </div>
        </>
